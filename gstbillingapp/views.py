...
@login_required
def chatbot(request):
    return render(request, 'gstbillingapp/chatbot.html')
...
def invoice_create(request):
    ...
    if request.method == 'POST':
        ...
        # Use a mutable copy so we can safely access optional fields without
        # risking MultiValueDictKeyError when some inputs are hidden/disabled
        # (e.g. Cash Customer mode).
        ...
        # Safely read customer fields; these may legitimately be blank in
        # "Cash Customer" mode, so we always default to empty strings.
        customer_name = invoice_data.get('customer-name', '')
        customer_address = invoice_data.get('customer-address', '')
        customer_phone = invoice_data.get('customer-phone', '')
        customer_gst = invoice_data.get('customer-gst', '')
